...
4. **质量损失**：小字号（10-14px）失去清晰度，因为它们是从低分辨率源放大的
...
**关键优势**：使用 WASM 解析器，你可以使用 Canvas 原生文本渲染**直接在目标缩放级别渲染文本**。虽然小字号在大幅缩放时仍会显示一些模糊（这是基于位图的 Canvas 渲染的限制），但质量**明显优于 SVG foreignObject**，因为你在每个缩放级别都重新渲染文本，而不是缩放预渲染的位图。这对于 **小字号（10-14px）** 尤其重要，这些字号在 UI 文本、标签和注释中很常见。
...
|------|------------------|-------------------|
| **系统字体** | ✅ 自动访问 | ⚠️ 必须手动加载字体 |
| **字体设置** | ✅ 无需设置 | ⚠️ 需要显式加载字体 |
| **小字号清晰度（10-14px）** | ❌ 缩放后非常模糊 | ✅ 缩放后更清晰 |
...
**关键优势**：SVG foreignObject 可以自动使用系统字体，无需任何设置，这使得它在简单用例中非常方便。
...
**权衡**：HTML Layout Parser 需要**手动加载字体**（必须显式加载字体文件），而 SVG foreignObject 可以自动使用系统字体。这使得 SVG foreignObject 在简单场景下更方便，但 HTML Layout Parser 提供了更好的控制和跨平台一致性。

::: tip 重要建议
**如果你的应用运行在主线程（非 Web Worker/Node.js 环境）**，我们**强烈推荐优先使用 SVG foreignObject 方案**。它更简单、无需加载字体、开箱即用。

只有在以下场景时才考虑使用 HTML Layout Parser：
- ✅ 需要在 **Web Worker** 中渲染（SVG foreignObject 不支持）
- ✅ 需要在 **Node.js** 环境中渲染
- ✅ 需要**较大缩放**且对小字号清晰度要求高
- ✅ 需要完全控制渲染过程和字体
:::
...
**根据需求选择**：
- **SVG foreignObject**：简单场景、大字号、仅使用系统字体、无较大缩放
- **HTML Layout Parser**：需要**缩放时更好的小文本质量**、**自定义字体**或**较大缩放**的应用
